...
live on the `data` volumeClaimTemplate (64Gi, `longhorn-capacity` on nishir)
...
DB_DATABASE_NAME_FILE); it also reaches immich-valkey :6379 and immich-ml :3003.
The NetworkPolicy admits only tailscale-system on http and vmagent on metrics.
...
- `overlays/nishir/` — `immich` Secret (.enc.env), PVC swap to `immich-data` +
  `timeline-data`, monitoring component, QSV/AMD node affinity + `/dev/dri`
  mount for hardware transcoding (ffmpeg `accel: vaapi` in the server config).
- `overlays/nishir-tailnet/` — Tailscale Ingress, SOPS `config.yaml`
  (externalDomain immich.i.shikanime.studio, ffmpeg VAAPI, SMTP, Dex OAuth) as
