...
(:8644), each published as a tailscale Ingress (`automata`, `nishir`,
`automata-a2a`, `automata-dashboard`, `automata-webhook`).
...
- `components/` — a2a, api, browser, dashboard, honcho, webhook; each patches
  the sts (plus svc/netpol where it exposes a port).
