...
Auto-mirror of GitHub orgs/repos into Forgejo: a single-replica StatefulSet
polling `GITEA_URL` (forgejo.shikanime.svc.cluster.local) into the
...
- `overlays/nishir-tailnet/` — Tailscale Ingress, netpol/sts patches (Tailscale
  header auth, `BETTER_AUTH_*`), secretGenerator from `gitea-mirror/.enc.env`.
