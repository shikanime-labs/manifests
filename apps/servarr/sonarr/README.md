...
(svc `sonarr`); the TLS component re-exposes it on https :9898, admitted
...
- `base/` — StatefulSet (http :8989, `/ping` probes, rclone init container
  ...
  (4Gi), NetworkPolicy (ingress from prowlarr, bazarr, seerr, jellyfin),
  VPA.
...
- `overlays/nishir/` — Certificate (nishir ClusterIssuer), config.xml and
  pkcs12-password SOPS Secrets, mounts `shows-data` and
  `downloads-sonarr-data`.
