...
Lidarr (lscr.io/linuxserver/lidarr 3.1.0) manages the music library: it
queries prowlarr for releases and hands them to qbittorrent. Web UI on :8686
(svc `lidarr`); the TLS component re-exposes it on https :6868, which the
...
- `overlays/nishir/` — Certificate (nishir ClusterIssuer), config.xml and
  pkcs12-password SOPS Secrets, mounts `music-data` and
  `downloads-lidarr-data`.
