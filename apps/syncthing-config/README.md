...
(`PATCH` on `/rest/config/gui` and `/rest/config/ldap`), keeping the auth
config in VCS the same way `qBittorrent.conf` and Jellyfin's `LDAP-Auth.xml`
are. Idempotent: safe to re-run (`kubectl -n shikanime delete job
syncthing-config`; Flux recreates it because the Kustomization owns the
object). Kubernetes Jobs are immutable, so a spec change lands only after the
old Job is deleted — Flux reports the drift until then.
