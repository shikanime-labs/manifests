...
Envoy AI Gateway serving OpenAI-schema inference. Each model is routed to its
own failover chain, local floors first: qwen/qwen3.8-27b → qwen-27b → nous →
openrouter; deepseek/deepseek-v4-flash → deepseek-flash → nous → openrouter;
z-ai/glm-4.7 → zai → nous → openrouter. Priority failover on 429/5xx.
...
- Local floors: plain HTTP `Backend` FQDNs. TODO when llama-cpp serves TLS:
  ...
  the floors.
- Remote providers: system-CA-validated TLS with explicit SNI
  (inference-api.nousresearch.com, openrouter.ai).
...
- `inference-zai/.enc.env` → Secret `inference-zai`
