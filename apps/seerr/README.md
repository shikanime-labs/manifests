...
Seerr (ghcr.io/seerr-team/seerr v3.4.1) is the request front-end for the media
stack: users request movies and shows that radarr and sonarr then fulfill, and
jellyfin can reach it for deep links. Web UI on :5055 (svc `seerr`) behind a
tailscale Ingress; config on PVC `seerr-config` (512Mi).
