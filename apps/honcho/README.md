...
`honcho-postgres.shikanime.svc.cluster.local:5432`; an alembic init container
migrates the schema before the app starts. Netpol admits only hermes-agent pods
and vmagent.

## Layout

- `base/` — Deployment (alembic-upgrade initContainer, health probes), Service
  (:8000), HPA, VPA, netpol.
